...
if not exist public-key.asc (
  echo %underline%%bright_red%Require public-key.asc to be in ^%CD^%.%clear_style%
  exit /b 1
)
...
gpg --import public-key.asc private-key.asc
...
mkdir C:\Users\%USERNAME%\prog\personal
...
:: TODO(Ryan): Update with correct URLs $
powershell -c "Invoke-WebRequest http://www.example.com/package.zip -OutFile .gvimrc"
powershell -c "Invoke-WebRequest http://www.example.com/package.zip -OutFile .vimrc"
